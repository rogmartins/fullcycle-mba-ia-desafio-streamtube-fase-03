...
describe('Auth (e2e)', () => {
  ...
  async function registerConfirmAndLogin(
    ...
    const res = await request(app.getHttpServer())
      ...
      .send({ email, password });
    return {
      access_token: (res.body as AuthSuccessResponseBody).access_token!,
      refresh_token: (res.body as AuthSuccessResponseBody).refresh_token!,
    };
  }
  ...
  describe('POST /auth/register', () => {
    it('returns 201 with { id, email } on valid registration', async () => {
      ...
      expect((res.body as AuthSuccessResponseBody).id).toBeDefined();
      expect((res.body as AuthSuccessResponseBody).email).toBe(
        'user@example.com',
      );
    });
  ...
  describe('JWT Guard', () => {
    ...
    it('returns 200 on GET /auth/me with a valid access token', async () => {
      ...
      expect((res.body as AuthSuccessResponseBody).sub).toBeDefined();
      expect((res.body as AuthSuccessResponseBody).email).toBe(
        'me@example.com',
      );
    });
  ...
  describe('POST /auth/login', () => {
    ...
    it('returns 200 with access_token and refresh_token on valid credentials', async () => {
      ...
      expect((res.body as AuthSuccessResponseBody).access_token).toBeDefined();
      expect((res.body as AuthSuccessResponseBody).refresh_token).toBeDefined();
      expect(typeof (res.body as AuthSuccessResponseBody).access_token).toBe(
        'string',
      );
      expect(typeof (res.body as AuthSuccessResponseBody).refresh_token).toBe(
        'string',
      );
    });
  ...
  describe('POST /auth/refresh', () => {
    it('returns 200 with new access_token and refresh_token on valid refresh token', async () => {
      ...
      expect((res.body as AuthSuccessResponseBody).access_token).toBeDefined();
      expect((res.body as AuthSuccessResponseBody).refresh_token).toBeDefined();
      expect((res.body as AuthSuccessResponseBody).refresh_token).not.toBe(
        refresh_token,
      );
    });
  ...
  describe('POST /auth/reset-password', () => {
    it('returns 204 with a valid token and updates the password', async () => {
      ...
      const loginRes = await request(app.getHttpServer())
        ...
        .expect(200);
      expect(
        (loginRes.body as AuthSuccessResponseBody).access_token,
      ).toBeDefined();
    });
